...
download (){
	...
	echo "------ extract $XTOOLS"
	tar xpf $CWD/$BUILD/$SOURCE/$XTOOLS.tar.?z* -C "$CWD/$BUILD/$SOURCE/" || exit 1
	
	echo "------ download $BOOT_LOADER"
	if [ -d $CWD/$BUILD/$SOURCE/$BOOT_LOADER ]; then
	    cd $CWD/$BUILD/$SOURCE/$BOOT_LOADER $CWD/$BUILD/$SOURCE/$BOOT_LOADER && git pull origin HEAD && cd $CWD
	else
	    git clone $URL_BOOT_LOADER_SOURCE/${BOOT_LOADER}.git $CWD/$BUILD/$SOURCE/$BOOT_LOADER || exit 1
	fi
	...
	if [ "$BOARD_NAME" = "cubietruck" ]; then
		echo "------ Download $SUNXI_TOOLS"

		cd $CWD/$BUILD/$SOURCE/$BOOT_LOADER && git checkout $BOOT_LOADER_VERSION && cd $CWD

		if [ -d $CWD/$BUILD/$SOURCE/$SUNXI_TOOLS ];then
		    cd $CWD/$BUILD/$SOURCE/$SUNXI_TOOLS && git pull origin HEAD && cd $CWD
		else
		    git clone $URL_SUNXI_TOOLS/$SUNXI_TOOLS $CWD/$BUILD/$SOURCE/$SUNXI_TOOLS
		fi


		if [ ! -f $CWD/$BUILD/$SOURCE/$FIRMWARE1 ]; then
		    echo "------ Download $FIRMWARE1"
		    wget -c --no-check-certificate $URL_FIRMWARE/$FIRMWARE1 -O $CWD/$BUILD/$SOURCE/$FIRMWARE1
		fi
		if [ ! -f $CWD/$BUILD/$SOURCE/$LINUX_CONFIG ]; then
		    echo "------ Download $LINUX_CONFIG"
		    wget --no-check-certificate $URL_LINUX_CONFIG_SOURCE/$LINUX_CONFIG -O $CWD/$BUILD/$SOURCE/$LINUX_CONFIG
		fi
	fi
}
...
compile_sunxi_tools (){
	echo "------ Compiling $SUNXI_TOOLS"
	cd $CWD/$BUILD/$SOURCE/$SUNXI_TOOLS
	# for host
	make -s clean && make -s fex2bin && make -s bin2fex || exit 1
#	cp fex2bin bin2fex /usr/local/bin/
	# for destination
	make -s clean && make $CTHREADS 'fex2bin' CC=${CROSS}gcc || exit 1
...
#		make $CTHREADS ARCH=arm CROSS_COMPILE=$CROSS firefly-rk3288.dtb || exit 1
		...
		echo A | unzip -o $CWD/$BUILD/$SOURCE/$FIRMWARE -d $CWD/$BUILD/$SOURCE/$LINUX_SOURCE/firmware || exit 1

		# use proven config
		install -D $CWD/$BUILD/$SOURCE/$LINUX_CONFIG $CWD/$BUILD/$SOURCE/$LINUX_SOURCE/.config
...
setenv bootargs 'console=tty0,115200 root=/dev/$ROOT_DISK ro rootwait rootfstype=ext4 sunxi_ve_mem_reserve=0 sunxi_g2d_mem_reserve=0 sunxi_no_mali_mem_reserve sunxi_fb_mem_reserve=16 hdmi.audio=EDID:0 disp.screen0_output_mode=1920x1080p60 panic=10 consoleblank=0 enforcing=0 loglevel=1'
#--------------------------------------------------------------------------------------------------------------------------------
...
EOF
	    ...
	    $CWD/$BUILD/$SOURCE/$BOOT_LOADER/tools/mkimage -C none -A arm -T script -d $CWD/$BUILD/$PKG/kernel-${_KERNEL}/boot/boot.cmd "$CWD/$BUILD/$PKG/kernel-${_KERNEL}/boot/boot.scr" || exit 1
	    install -Dm644 "$CWD/$BUILD/$SOURCE/$BOOT_LOADER/$BOOT_LOADER_BIN" "$CWD/$BUILD/$PKG/kernel-${_KERNEL}/boot/$BOOT_LOADER_BIN"

	    if [ "$NEXT" == "next" ];then
		install -Dm644 $CWD/$BUILD/$SOURCE/$LINUX_SOURCE/arch/$_ARCH/boot/dts/sun7i-a20-cubietruck.dtb "$CWD/$BUILD/$PKG/kernel-${_KERNEL}/boot/dtb/sun7i-a20-cubietruck.dtb" || exit 1
...
if [ "\$XFCE" = "true" ]; then
    echo "------ flash linuxroot ${ROOTFS_XFCE}-$BOARD_NAME-build-${VERSION}.img"
    $TOOLS/rkflashtool w linuxroot < ${ROOTFS_XFCE}-$BOARD_NAME-build-${VERSION}.img || exit 1
else
    echo "------ flash linuxroot ${ROOTFS}-$BOARD_NAME-build-${VERSION}.img"
    $TOOLS/rkflashtool w linuxroot < ${ROOTFS}-$BOARD_NAME-build-${VERSION}.img || exit 1
fi
echo "------ reboot device"
