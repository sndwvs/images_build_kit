...
create_bootloader_pack(){
    ...
    [[ ! -z $BLOB_LOADER ]] && install -Dm644 "$CWD/$BUILD/$SOURCE/$RKBIN/bin/${SOCFAMILY:0:4}/$BLOB_LOADER" "$CWD/$BUILD/$OUTPUT/boot/$BLOB_LOADER" >> $CWD/$BUILD/$SOURCE/$LOG 2>&1 || (message "err" "details" && exit 1) || exit 1
