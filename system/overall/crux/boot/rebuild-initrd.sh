...
if [[ -x /sbin/mkinitrd ]]; then
    ...
    popd
elif [[ -x /sbin/mkinitramfs ]]; then
    mkinitramfs -k ${KERNEL_VERSION} -m ${INITRD_MODULES}
    mv /tmp/initrd.img-${KERNEL_VERSION} /tmp/initrd-${KERNEL_VERSION}.img
fi
...
if [[ ! -z $(mount | grep -P "(mmcblk0|sd).1.*boot.*fat") ]]; then
