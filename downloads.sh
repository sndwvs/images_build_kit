...
download (){
    ...
    if [ "$BOARD_NAME" = "firefly" ];then
        ...
        if [[ $KERNEL_SOURCE == "next" ]]; then
            ...
            message "" "extract" "$BARANCH"
            cd $CWD/$BUILD/$SOURCE/$LINUX_SOURCE && git checkout $BARANCH >> $CWD/$BUILD/$SOURCE/$LOG 2>&1 || (message "err" "details" "$BUILD/$SOURCE/$LOG" && exit 1) || exit 1
        fi
    fi
