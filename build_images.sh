...
setting_fstab() {
    if [[ ! $(cat $SOURCE/$ROOTFS/etc/fstab | grep $ROOT_DISK) ]];then
        ...
        if [[ $DISTR == sla* ]]; then
            ...
            [[ $SOCFAMILY == rk356* ]] && echo "/dev/mmcblk0p2    /boot      ext4    noatime,nodiratime       0       1" >> $SOURCE/$ROOTFS/etc/fstab
...
setting_system() {
    ...
    rsync -avL --chown=root:root $CWD/system/overall/$DISTR/ $SOURCE/$ROOTFS/ >> $LOG 2>&1 || (message "err" "details" && exit 1) || exit 1
