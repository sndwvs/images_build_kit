...
#    [[ $KARCH == arm64 ]] && local CROSS=$CROSS64
    ...
    if [[ $SOCFAMILY == rk3* ]]; then
        ...
        # for rockpro64, rock pi 4, pinebook pro, rock64, firefly-rk3399
        if [[ $BOARD_NAME == rockpro64 || $BOARD_NAME == rock_pi_4 || $BOARD_NAME == pinebook_pro || $BOARD_NAME == rock64 || $BOARD_NAME == firefly_rk3399 ]]; then
