...
compile_boot_loader() {
    ...
    if [[ $SOCFAMILY == rk3* ]]; then
        ...
        # for rockpro64, rock pi 4, pinebook pro, rock64, firefly-rk3399, rock pi e, helios64, orange pi 4, station m1, station p1
        if [[ $BOARD_NAME == rockpro64     || $BOARD_NAME == rock_pi_4*     || $BOARD_NAME == pinebook_pro || \
              ...
              $BOARD_NAME == helios64      || $BOARD_NAME == orange_pi_4*   || $BOARD_NAME == station_*1 ]]; then
